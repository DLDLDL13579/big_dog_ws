...
**16-DOF 轮腿式机械狗（Mini Cheetah 构型）**，Jetson Orin NX 大脑 + UpBoard 小脑。FAST-LIO2 建图 → ICP 3D 全局定位 → Nav2 3D 导航 → TCP 小端指令 → UpBoard RL 策略驱动运动。**全链路已打通并实测验证，navigate_to_pose 端到端已跑通，导航参数已调优。**
...
## 2. 网络配置（★ 2026-08-25 更新）

| 接口 | IP | 用途 | 状态 |
|------|-----|------|------|
| `wlP1p1s0` (WiFi) | **192.168.31.91/24**（静态） | 主网络 / SSH / 上网 | ✅ UP |
| `enx00e04c680779` (USB 网卡) | **192.168.1.100/32** | Mid-360 雷达直连 | ✅ UP |
| `enP8p1s0` (有线) | 10.0.0.48/24 | UpBoard 底盘直连 | ✅ UP |

**WiFi 单 IP 配置（nmcli 已持久化）：**
```
ipv4.method: manual
...
**⚠️ 教训（2026-08-25）：** 之前 WiFi 同时配 192.168.1.100/24 辅助 IP，与 USB 网卡 IP 冲突，导致雷达数据链路建不起来（驱动握手成功但无点云）。已移除 WiFi 辅助 IP，雷达改走 USB 网卡直连。

| 设备 | IP | 通信方式 |
|------|-----|--------|
| UpBoard 小脑 | 10.0.0.6 | LCM 组播 (239.255.76.67:7667, ifname=enP8p1s0) + TCP :3333 |
| Mid-360 雷达 | 192.168.1.195 | USB 网卡 192.168.1.100 直连（数据端口 56301/56401） |
| D435i 相机 | — | USB3 直连 |
...
| 文件 | 大小 | 位置 | 说明 |
|------|------|------|------|
| `lab_3d_map.pcd` | **16 MB** | `src/dog_brain/maps/` | 导航用 PCD 地图（2026-08-25 更新） |
| `test.pcd` | 16 MB | `dog_ws/` | FAST-LIO2 默认存图路径 |
| `map_20260825_1942.pcd` | 16 MB | `dog_ws/` | 2026-08-25 建图快照（已归档） |
| `map_fresh_20260820_175432.pcd` | 7 MB | `dog_ws/` | 历史建图快照 |
| `map_v2_20260820_192409.pcd` | 7 MB | `dog_ws/` | 历史建图快照 |

**三车地图共享（2026-08-25 已实现）：**
- 主车：`~/wheeltec_ros2/src/wheeltec_robot_rtab/my_map.pgm/yaml`
- robot1：`~/robot1_ws/src/robot1_nav/maps/lab_map.pgm/yaml`
- 机械狗：`~/dog_ws/maps/` + `src/dog_brain/maps/lab_3d_map.pcd`
...
| SLAM 建图 | **90%** | FAST-LIO2 稳定建图、存图服务可用、多张 PCD 产出、相机可关闭省电 |
| ICP 3D 定位 | **85%** | 四件套链路打通、fitness=1.000、多次重定位验证 |
| Nav2 导航 | **85%** | 全链路激活、端到端验证、参数调优；**体素层 z 窗口过窄待修** |
| 控制链 | **95%** | TCP 小端已修、注入已部署、实测驱动狗体运动 ✅ |
| 多机协同 | **90%** | robot2 接入 domain_bridge、6 话题双向桥接、三车地图共享已实现 |
| 系统加固 | **40%** | systemd 建图已部署（当前 disabled）、导航 systemd 未建、日志持久化已启用 |
| 文档体系 | **95%** | 6 份完整文档（README + 架构 + 完整文档 + 启动指南 + OPS_GUIDE + PROJECT_STATUS） |
| 楼梯间专项 | **0%** | 未开始 |
| 综合 | **~70%** | |
...
| # | 项目 | 说明 |
|---|------|------|
| 1 | ~~发 goal 实测驱动狗体运动~~ | ✅ 已完成，nav_goal_test.py 端到端验证 |
| 2 | ~~UpBoard TCP 注入部署~~ | ✅ 已完成，rt_rc_interface.cpp 已部署 |
...
| 7 | **Nav2 体素层 z 窗口修复** | 当前 -0.2~1.4m，趴下时雷达在 -2m 导致避障失明，需按全姿态范围配置 |

### 🟢 P2 — 优化项
| # | 项目 | 说明 |
|------|------|------|
| 8 | 失同步看门狗 | 雷达掉线重连后时钟跳变 → 自动 restart |
| 9 | UpBoard 日志清理 | 689MB 日志占空间 |
| 10 | 建图漂移优化 | 长时间建图的漂移控制 |
| 11 | git push | 3 批次已提交未推送（需用户批准） |
| 12 | 雷达断电恢复 | 多次崩溃后雷达数据链路冻结，需断电重启（待查根因） |
...
7. **Nav2 体素层 z 窗口过窄** — 趴下时雷达传感器 z 落到 -2m，超出窗口 -0.2~1.4m，避障失明
8. **雷达数据链路偶发冻结** — 多次崩溃/重启后雷达握手成功但无点云，需断电重启
...
> **2026-08-25 新增：** 多机协同 robot2 接入完成、三车地图共享已实现、建图/导航模式切换验证通过。  
> **当前阻塞：** Nav2 体素层 z 窗口过窄导致避障失效（趴下时雷达点云被丢弃），需修复参数。  
> 下一步：修复体素层参数 → 导航 systemd 服务 → 全向横移扩展 → 楼梯间专项（远期）。
