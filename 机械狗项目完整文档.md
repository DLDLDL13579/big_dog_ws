...
| Jetson WiFi 辅助 IP | wlP1p1s0 | 192.168.1.100/24（nmcli 已配） | Mid-360 雷达 192.168.1.195 |
| Jetson 主板网口 | enP8p1s0 | 10.0.0.48/24 | UpBoard 10.0.0.6 |
| UpBoard | enp0s25 | 10.0.0.6 | Jetson 10.0.0.48 |
| 雷达 | - | 192.168.1.195 | Jetson WiFi 辅助 IP 192.168.1.100 |
...
| UpBoard | xjzx | xjzx | 经 Jetson 跳板：`sshpass -p xjzx ssh -J nvidia@192.168.1.100 xjzx@10.0.0.6` |
...
**铁律**：LCM 用大端 `'>12f'`（协议规定勿动），TCP 用小端 `'<3d'`（匹配 UpBoard x86 架构）。

### 1.4 网络路由配置

```
# WiFi 双 IP（nmcli 已持久化，重启自动生效）
...
ROS2 → TCP:
  ...
  struct.pack('<3d', flag, omega, v_norm)   # 小端(2026-08-18 修复)
...
模式B navigation (navigation_launch.py, 已验证全链路激活):
  ...
  └── Nav2 (nav2_bringup, nav2_3d_params.yaml): Navfn + DWB + behavior + VelSmoother(OPEN_LOOP)
        global_costmap(static map_topic=/elevation→/map + voxel + inflation)
...
dog_brain (navigation_launch.py)
  ...
  │ global_costmap: static(map_topic=/map) + voxel + inflation(0.45)│
  │ local_costmap:  voxel(/cloud_registered_body) + voxel(camera) + inflation│
  │ velocity_smoother: feedback=OPEN_LOOP  max [0.45,0,0.35]       │
  └──────────────────────────────────────────────────────────────┘
...
| freq_localization | 1.0 Hz | ICP 精匹配频率 |
...
- linuxptp 3.1.1 已装但用不上。**此硬件 PTP 不可行**。
...
| 路径 | 协议 | 格式 |
|------|------|------|
| 10.0.0.6:3333 | TCP | `struct.pack('<3d', flag, ω_z, v_x/0.75)` 小端 |
...
当前限制 (能力错配, 待扩展):
  1. Nav2 差分配置: max_vel_y=0.0 / acc_lim_y=0.0
...
| 参数 | 值 | 来源 |
|------|-----|------|
| 最大直行速度 | 0.45 m/s | Nav2 DWB max_vel_x（原 0.75，已降速防飘移） |
| 最大角速度 | 0.35 rad/s | Nav2 DWB max_vel_theta（原 0.8，已降速防飘移） |
...
Z 0.0m ┤═════════ 地面 ═════════════
       ╔ Inflation Layer  radius 0.45m, cost_scaling 2.5, 迫使 DWB 提前绕行 ╗
...
| ICP 定位 | 定位四件套、fitness 1.000、1Hz精匹配、z归零、多次重定位 | 6/6 ✅ |
...
6. **全向横移能力未启用**——Nav2 差分配置(vy=0) + TCP 无 vy 槽位，横移指令传不到 UpBoard。
...
| 9 | UpBoard TCP 控制解锁 | 🟡 已改待部署 | rt_rc_interface.cpp TCP 注入已写(SWF双源+300ms看门狗), 字节序已修, 未部署未实测 |
| 10 | Nav2 全链路激活 | ✅ 已解决 | 修 5 类 bug，9 服务器全 active，ICP fitness=1.0 |
| 11 | STARTUP_GUIDE.md 过时 | 🟡 待办 | 仍写雷达 .12/.50（本架构文档已为最新） |
| 12 | 发 goal 实测驱动 | 🔴 待办 | /cmd_vel→TCP→UpBoard 驱动狗未实测，需狗在场 |
...
| 2 | 轮腿复合全向(2026-08-18 修正) | 轮=前进/后退, 腿=转向/横移, 本体支持 vy; 但 Nav2/TCP 仍差分, 全向待扩展 |
...
6. **TCP 注入解锁（本地副本, 未部署）**：rt_rc_interface.cpp RL 分支加 SWF 双源切换 + 300ms 超时看门狗自动停车。
...
| global_costmap 全黑 | voxel z 范围不够，传感器原点高于栅格顶 | z_resolution 0.10 / z_voxels 24 |
...
| global_map_publisher 性能灾难 | points.tolist() 每周期转 58 万点 CPU 打满 | 一次性缓存 PointCloud2 + numpy.tobytes() |
