...
### 一句话
全链路已打通并实测，navigate_to_pose 端到端跑通；导航参数经 R1~R4 调优（其中 R1 为首次真正生效）。
...
- 分支：main，**领先 origin/main 4 个提交**（最新 28c5616 = R4 体素层），工作树干净
- ⚠️ 这 4 个未推送提交含 08-27 工作；**是否 git push 需用户批准**（安全红线，不擅自 push）
...
| ICP 3D 定位 | 85% | 四件套打通、fitness=1.0、多次重定位 |
| Nav2 导航 | 88% | 全链路 + 端到端 + R1~R4 调优（R1 首次生效） |
...
- ✅ 体素层 z 窗口阻塞已解除：R4 收到 -0.75~1.65m、垂直分辨率 0.15，实测闭环误差 0.17m（旧文档仍记为阻塞，已过期）
- ★ DWB critics 首次真正生效（R1）：旧平铺写法从未生效，已改嵌套写法；历史「调优已完成」结论对这些 critic 作废
- ⚠️ 失同步仍不自愈：雷达掉线重连后时钟跳变，需手动 restart

### 下一步（按优先级）
1. 导航常驻 systemd（dog-nav.service）
2. 全向横移扩展（Nav2 全向 + TCP 加 vy + UpBoard v_des[1]）
3. 失同步看门狗、UpBoard 日志清理（689MB）
4. 楼梯间专项（远期，约 10-14 周）
5. 待用户批准后推送 4 个提交
...
### ✅ 已完成要点
硬件集成（三网口+LCM+TCP）、传感器驱动（Mid-360/D435i/时间戳/外参）、SLAM 建图、ICP 3D 定位（替换 AMCL）、Nav2 3D 导航（全链路+端到端+R1~R4 调优）、控制链（TCP 小端+注入部署+实测驱动）、多机协同（三车地图共享）、文档体系。

### 🟡 P1 重要待办
1. 导航常驻 systemd（建 dog-nav.service，开机自启）
2. 全向横移扩展（Nav2 全向 + TCP 加第 4 个 double=vy + UpBoard 注入 v_des[1]）
3. 建图模式 systemd 启用（当前 disabled，需 enable dog-brain）

### 🟢 P2 优化项
1. 失同步自动看门狗（连续 N 秒 No Effective Points → 自动 restart）
2. UpBoard 日志清理（689MB）
3. 建图漂移优化
4. RViz 远程降采样（/Laser_map_downsampled）
