...
## ★ 当前真实状态（2026-08-27，覆盖各旧状态文档）

### 一句话
全链路已打通并实测，navigate_to_pose 端到端跑通；导航参数经 R1~R4 调优（R1 首次真正生效）；2026-08-28 B1 时间戳修改已回退（R7），回退后全链路延迟 21~43ms，z/pitch 稳定，5分钟运动无失效；遗留 planner_server goal 时间戳过旧的少量 extrapolation（不影响 costmap）。
...
### Git 真实状态（2026-08-27 实测，纠正旧文档）
- 提交数：**30**（旧文档记 18 / 21 均已过期）
- 分支：main，**领先 origin/main 4 个提交**（最新 28c5616 = R4 体素层），工作树含 B1 回退修改（R7，2026-08-28）
- ⚠️ 这 4 个未推送提交 + R7 本地修改含 08-27/08-28 工作；**是否 git push 需用户批准**（安全红线，不擅自 push）
...
| Nav2 导航 | 85% | 全链路 + 端到端 + R1~R4 调优；B1 回退后延迟 21~43ms，5分钟运动无失效 |
| 控制链 | 95% | TCP 小端已修、注入已部署、实测驱动狗体 |
...
### 下一步（按优先级，2026-08-27 更新）
1. ~~★ z 漂移根因修复（P0）~~ ✅ 已解决（R7）：回退 B1 后 z/pitch 稳定，无需约束 ICP 旋转
2. 导航启动问题排查（P1）：publish_initial_pose 崩溃、livox 偶发无数据
3. ~~B1 第二阶段（P2）~~ ❌ 已取消：会引入与 B1 相同的延迟问题
4. 导航常驻 systemd（dog-nav.service）
5. 全向横移扩展（Nav2 全向 + TCP 加 vy + UpBoard v_des[1]）
6. 失同步看门狗、UpBoard 日志清理（689MB）
7. 待用户批准后推送 4 个提交 + R7 本地修改
...
| 偶发 planner 规划失败 | 瞬时，recovery 后恢复 🟡 |
| RL 运动模型 | 源码分析完成，待实车标定 🔍 |
| 站立零速前进 | 排查中，A/B 已排除 🔍 |

### 暂不修改的参数（新增）
- 非线性速度映射 NAV_V_MIN（与 deadband、RL 训练数据深度耦合，需先标定）
...
- R9：文档更新（RL运动模型分析 + deadband确认 + 站立前进排查），无代码修改
- 本地领先 origin/main 约 9 个提交，未 push
